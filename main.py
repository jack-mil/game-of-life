...
def runGame():
    ...
    screen = pygame.display.set_mode(
        (settings.screen_width, settings.screen_height))  # rel.it runs 800x600
    pygame.display.set_caption("The Game of Life")
    screen.fill(WHITE)

    for x in range(settings.screen_width):
        for y in range(settings.screen_height):
            rect = pygame.Rect(x * settings.block_size,
                               y * settings.block_size, settings.block_size,
                               settings.block_size)
...
def getRectFromMouse():
  ...
  """
    ...
    posX = math.floor(pos[0] / settings.block_size
                      ) * settings.block_size  # rounds to nearest block corner
    posY = math.floor(pos[1] / settings.block_size) * settings.block_size
    square = pygame.Rect(posX, posY, settings.block_size, settings.block_size)
    return square
